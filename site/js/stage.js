...
export function mountStage(canvas) {
  ...
  // Tap an analyser that is already part of a Web Audio graph (the site's loop).
  // Passing null goes back to the idle wave.
  function tap(node) {
    analyser = node;
    if (node) freq = new Uint8Array(node.frequencyBinCount);
  }
